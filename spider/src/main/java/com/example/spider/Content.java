    ...
    public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
        ...
        System.out.println("page.getHeight()/textView.getLineHeight()"+page.getHeight()/textView.getLineHeight());
            ...
            if (current_length + page.getHeight() / textView.getLineHeight()+3 < total_length) {
                ...
                bar_progress.setProgress(total_length);
            }
...
//加载进度条
        ...
        bar_progress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {             //设置进度
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {


               current_length=progress;
                System.out.println(current_length);
               Handler handler = new Handler();
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        page.scrollTo(0, textView.getLineHeight() *current_length );//在该run方法中执行滚动操作
                    }
